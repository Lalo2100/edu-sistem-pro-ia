...
function buildPrompt({tipo,nivel,grado,area,tema,duracion,indicaciones}) {
  const names = {
...
Completá las secciones pedagógicas que correspondan. Para planificación o secuencia, considerá cuando sea pertinente:
...
app.post("/api/generar", async (req,res) => {
  ...
  if(!apiKey) {
    return res.status(503).json({error:"La IA no está configurada. Agregá GEMINI_API_KEY en las variables de entorno de Vercel."});
  }
  ...
  try {
    ...
    if(!r.ok) {
      ...
      return res.status(502).json({error:j?.error?.message||"Gemini no pudo generar el contenido."});
    }
    const texto=(j?.candidates?.[0]?.content?.parts||[])
      .map(p=>p.text||"").join("").trim();
    if(!texto) return res.status(502).json({error:"Gemini no devolvió contenido. Probá nuevamente."});
    res.json({ok:true,texto});
  } catch(err) {
    console.error("Generation error",err);
    res.status(500).json({error:"No se pudo conectar con Gemini. Revisá la configuración del servidor."});
  }
});
...
if(require.main===module) {
  app.listen(PORT,()=>console.log(`Edu.sistem pro ia funcionando en http://localhost:${PORT}`));
}
